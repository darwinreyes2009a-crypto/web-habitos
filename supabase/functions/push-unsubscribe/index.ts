...
// Elimina la suscripción push (endpoint) del usuario autenticado.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!endpoint) return new Response(JSON.stringify({ error: 'falta endpoint' }), { status: 400, headers: cors });
